...
static void rib_refresh_l3vni_rmac_table(struct hash_bucket *bucket, void *arg)
{
	...
	struct zebra_if *zif = zl3vni->vxlan_if->info;
...
static void log_lsp_refresh(struct zebra_lsp *lsp) {
    if (!lsp->best_nhlfe) {
        zlog_debug("Omitting refresh of LSP for label %u: no best nhlfe", lsp->ile.in_label);
    } else {
        char nh_buff[BUFSIZ] = "";
        ...
        zlog_debug("Refreshing LSP for label: %u (num-ecmp: %u) best is next-hop: %s labels: %s", lsp->ile.in_label, lsp->num_ecmp, nh_buff, labels_buff);
    }
}
...
static void rib_refresh_lsp(struct hash_bucket *bucket, void *arg) {
    ...
    if (!lsp->best_nhlfe) {
        zlog_debug("Omitting refresh of LSP for label: %u num-ecmp: %u: no best nhlfe", lsp->ile.in_label, lsp->num_ecmp);
    } else {
        log_lsp_refresh(lsp);
        dplane_lsp_add(lsp);
    }
